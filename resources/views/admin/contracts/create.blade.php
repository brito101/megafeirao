...
<section class="dash_content_app">
    ...
    <header class="dash_content_app_header pb-1">
        <h2 class="icon-file-text">Novo Contrato</h2>
            ...
            <div class="nav_tabs_content">
                <div id="parts">
                    <form action="{{ route('admin.contracts.store') }}" method="post" class="app_form">
                        ...
                        <input type="hidden" name="automotive_persist" value="{{ old('automotive') }}">
                                ...
                                <div class="label_g2">
                                    <label class="label">
                                        ...
                                        <select class="select2" name="owner"
                                            ...
                                            <option value="{{ $seller->id }}" {{ (old('owner') == $seller->id ? 'selected' : '') }}>{{ $seller->name }}
                                                ({{ $seller->document }})
                                            </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="label_g2">
                                    <label class="label">
                                        ...
                                        <select name="acquirer" class="select2"
                                            ...
                                            <option value="{{ $buyer->id }}" {{ (old('acquirer') == $buyer->id ? 'selected' : '') }}>{{ $buyer->name }}
                                                ({{ $buyer->document }})
                                            </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="app_collapse_content">
                                ...
                                <div class="label_g2">
                                    <label class="label">
                                        ...
                                        <input type="text" name="tribute" class="mask-money" placeholder="IPVA"
                                               value=""/>
                                    </label>
...
<script>
    ...
    $(function () {
        ...
        if ($('input[name="automotive_persist"]').val() > 0) {
            var property = $('select[name="automotive"]');
            $.post(property.data('action'), {property: $('input[name="automotive_persist"]').val()}, function (response) {
                setFieldProperty(response);
            }, 'json');
        }
    });
