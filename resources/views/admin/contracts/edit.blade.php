    ...
    <div class="dash_content_app_box">
        ...
        <div class="nav">
            ...
            <div class="nav_tabs_content">
                <div id="parts">
                    <form action="{{ route('admin.contracts.update', ['contract' => $contract]) }}" method="post" class="app_form">
                        @csrf
                        @method('PUT')
                        ...
                        <input type="hidden" name="automotive_persist" value="{{ old('automotive') ?? $contract->automotive }}">
                                ...
                                <div class="label_g2">
                                    <label class="label">
                                        ...
                                        <select class="select2" name="owner"
                                            ...
                                            @foreach($sellers->get() as $seler)
                                            <option value="{{ $seler->id }}" {{ (old('owner') == $seler->id ? 'selected' : ($contract->owner == $seler->id ? 'selected' : '')) }}>{{ $seler->name }}
                                                ({{ $seler->document }})
                                            </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="label_g2">
                                    <label class="label">
                                        ...
                                        <select name="acquirer" class="select2"
                                            ...
                                            @foreach($buyers->get() as $buyer)
                                            <option value="{{ $buyer->id }}" {{ (old('acquirer') == $buyer->id ? 'selected' : ($contract->acquirer == $buyer->id ? 'selected' : '')) }}>{{ $buyer->name }}
                                                ({{ $buyer->document }})
                                            </option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="app_collapse">
                            ...
                            <div class="app_collapse_content">
                                <label class="label">
                                    <span class="legend">Veículo:</span>
                                    <select name="automotive" class="select2"
                        ...
                        <div class="text-right mt-2">
                            <button class="icon-check-square-o btn btn-large btn-green">Editar Contrato</button>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    $(function () {
        ...
        function setFieldOwner(response) {
            ...
            // Properties
            $('select[name="automotive"]').html('');
            if (response.properties != null && response.properties.length) {
                $('select[name="automotive"]').append($('<option>', {
                    value: 0,
                    text: 'Não informar'
                }));
                ...
                $.each(response.properties, function (key, value) {
                    $('select[name="automotive"]').append($('<option>', {
                        ...
                        selected: ($('input[name="automotive_persist"]').val() != 0 && $('input[name="automotive_persist"]').val() == value.id ? 'selected' : false)
                    }));
                });
        ...
        $('select[name="automotive"]').change(function () {
            var property = $(this);
